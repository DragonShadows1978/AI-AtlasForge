...
class ValidationOrchestrator:
    ...
    def validate(
        ...
        evidence_sources: Optional[Dict[str, FetchedSource]] = None,
    ) -> ValidatedFindings:
        """
        ...
        Args:
            ...
            evidence_sources: Optional mapping of URL -> already captured source
                content from investigation-owned WebProxy artifacts. These are
                preferred over network refetching.
        ...
        self._log("Step 2: Loading/fetching source content...")
        ...
        missing_urls = []
        for url in urls:
            evidence_source = _lookup_evidence_source(url, evidence_sources)
            if evidence_source and evidence_source.accessible and evidence_source.content:
                sources[url] = evidence_source
                evidence_hits += 1
            else:
                missing_urls.append(url)
        if evidence_hits:
            self._log(f"  Loaded {evidence_hits}/{len(urls)} sources from pinned investigation evidence")
        if missing_urls:
            sources.update(self.source_fetcher.fetch_sources(missing_urls))
...
def _normalize_evidence_url(url: Optional[str]) -> str:
    """Normalize URLs enough to match claims to pinned WebProxy evidence."""
...
def _lookup_evidence_source(
    url: Optional[str],
    evidence_sources: Dict[str, FetchedSource],
) -> Optional[FetchedSource]:
    if not url or not evidence_sources:
        return None
    if url in evidence_sources:
        return evidence_sources[url]
    normalized = _normalize_evidence_url(url)
    if not normalized:
        return None
    for evidence_url, source in evidence_sources.items():
        if _normalize_evidence_url(evidence_url) == normalized:
            return source
    return None
...
def validate_subagent_findings(
    ...
    Args:
        ...
        progress_callback: Optional callback for progress updates
        evidence_sources: Optional investigation-owned source content by URL
