...
# =============================================================================
# PROVIDER API (Multi-Provider Model Switching)
# =============================================================================

@app.route('/api/provider/current')
def api_provider_current():
    """Get current provider configuration."""
    try:
        # Import provider_config here to avoid circular dependencies
        import provider_config

        # Get provider using provider_config (handles env var, mission.json, default)
        provider = provider_config.get_provider_config(MISSION_PATH)
        model_map = provider_config.MODEL_MAP.get(provider, {})
        pricing = provider_config.PRICING_MAP.get(provider, {})
        ...
        return jsonify({'error': str(e), 'provider': 'claude'}), 500


@app.route('/api/provider/set', methods=['POST'])
def api_provider_set():
    """Set current provider (updates mission.json)."""
    try:
        data = request.get_json()
        provider = data.get('provider', 'claude')

        # Validate provider
        valid_providers = {'claude', 'deepseek', 'gemini'}
        if provider not in valid_providers:
            return jsonify({'error': f'Invalid provider: {provider}'}), 400

        # Update mission.json with provider field
        def update_provider(mission):
            if not isinstance(mission, dict):
                mission = {}
            mission['provider'] = provider
            return mission

        io_utils.atomic_update_json(MISSION_PATH, update_provider, {})

        # Broadcast provider change via WebSocket
        socketio.emit('provider_changed', {'provider': provider})

        return jsonify({
            'success': True,
            'provider': provider,
            'message': f'Provider updated to {provider}'
        })
    except Exception as e:
...
def api_provider_stats():
    """Get provider usage statistics from mission logs."""
    try:
        from provider_analytics import get_provider_stats
        from pathlib import Path
        mission_logs_dir = Path(BASE_DIR) / "missions" / "mission_logs"
        return jsonify(get_provider_stats(mission_logs_dir, io_utils))
    except Exception as e:
        return jsonify({'error': str(e)}), 500
