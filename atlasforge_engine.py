...
class RDMissionController:
    ...
    def _get_enhancer(self):
        ...
        if not hasattr(self, '_enhancer'):
            self._enhancer = None
        if self._enhancer is None and AF_ENHANCER_AVAILABLE:
...
"""
    ...
    def _save_cycle_to_history(self, cycle_num: int, cycle_report: dict, continuation_prompt: str):
        ...
        if enhancer:
            try:
                af_report = enhancer.process_cycle_end(
                ...
                cycle_entry['af_enhancement'] = {
                    'drift_similarity': af_report.get('drift', {}).get('similarity'),
                    'drift_severity': af_report.get('drift', {}).get('severity'),
                    'healing_needed': af_report.get('drift', {}).get('healing_needed', False),
                    'exploration_added': af_report.get('exploration', {}).get('added', {}),
                    'bias_score': af_report.get('bias_analysis', {}).get('score')
                }
                # Update cycle_history with AtlasForge data
                self.mission["cycle_history"][-1] = cycle_entry
                logger.info(f"AtlasForge cycle processing: drift_similarity={af_report.get('drift', {}).get('similarity', 'N/A'):.2%}" if af_report.get('drift', {}).get('similarity') else "AtlasForge cycle processing complete")
            except Exception as e:
        ...
        if enhancer:
            try:
                ...
                if enhanced_prompt != continuation_prompt:
                    ...
                    self.mission['af_healing_applied'] = {
                        'cycle': current_cycle,
